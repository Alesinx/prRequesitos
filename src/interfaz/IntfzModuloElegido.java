...
public class IntfzModuloElegido {
	...
	private ArrayList<CurvaOriginal> listaDeCurvas; 		//curvas seleccionadas en la campa帽a
	...
	private void initialize(String nombreModulo) throws ClassNotFoundException {
		...
		String[] columnName = {"N","fecha y hora","Isc(A)","Voc(V)",
				"Pmax(W)","IPmax(A)","VPmax(V)","FF(%)","velViento","dirViento","Humedad","Temperatura","Irradiancia","RTD","C閘ula"};
...
//------PANEL CORRECCION
		...
		choiceMetodo.add("IEC60891 m茅todo 1");
		...
		txtfAlpha.setText(String.valueOf(mod.getAlfa()));	//valor por defecto del m贸dulo
		panelCorreccion.add(txtfAlpha);
		txtfAlpha.setColumns(10);

		txtfBetta = new JTextField();
		txtfBetta.setBounds(103, 168, 109, 20);
		txtfBetta.setText(String.valueOf(mod.getBeta()));	//valor por defecto del m贸dulo
		...
		txtfKappa.setText(String.valueOf(mod.getKappa()));	//valor por defecto del m贸dulo
		...
		btnCorregir_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(choiceMetodo.getSelectedItem().equals("IEC60891 m茅todo 1")) {
...
//---------PANEL GRAFICO
        ...
        String[] columaGraf = {"N","Tensi贸n(V)","Corriente(A)","Potencia(W)"};
